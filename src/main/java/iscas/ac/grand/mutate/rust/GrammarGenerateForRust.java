package iscas.ac.grand.mutate.rust;
...
//        str= " if "+str+"==0?"+ "{"+ 1+ "}"+ " else "+ "{"+ str+" }";
     ...
     * @return
     */
    public String generateSpecialPartForRust(String str, int index, Map<String, Integer> tokenAppearTimes, boolean ifNeedOutCycle, SymbolTable st, ScopeTree scopeNode,int lineOrder) {
        String result="";
        //方法返回值相关
        if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.methodDeclaration))){
            result=generateMethodDeclaration(str, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,lineOrder,false);
            return result;
        }

        //main方法相关
        if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.mainContent))){
      ...
      //模板函数相关
        if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.templateFunDeclaration))){
      ...
      //类声明相关
        if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.classDeclaration)||str.trim().equalsIgnoreCase(RustConfigureInfo.structDeclaration))){
        ...
        if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.fieldDeclaration)||str.trim().equalsIgnoreCase(RustConfigureInfo.localDeclaration))){//不区分大小写，如果是声明变量的文法串
            //声明文法特殊处理，记录类型，标识符，作用域等信息
            result=generateDeclaration(str,st,scopeNode,index,ifNeedOutCycle,tokenAppearTimes,lineOrder);
            return result;
        }else if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.fieldArrayDeclaration)||str.trim().equalsIgnoreCase(RustConfigureInfo.localArrayDeclaration))){//不区分大小写，如果是声明数组变量的文法串
            //声明文法特殊处理，记录类型，标识符，作用域等信息
            result=generateArrayDeclaration(str,st,scopeNode,index,ifNeedOutCycle,tokenAppearTimes,lineOrder);
            return result;
        }else if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.fieldPointerDeclaration)||str.trim().equalsIgnoreCase(RustConfigureInfo.localPointerDeclaration))){//不区分大小写，如果是声明指针变量的文法串
            //声明文法特殊处理，记录类型，标识符，作用域等信息
            result=generatePointerDeclaration(str,st,scopeNode,index,ifNeedOutCycle,tokenAppearTimes,lineOrder);
            return result;
        }
        //使用数组变量
        else if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.arrayElement))) {
        	String temResult=generateIdentifierByType(str,index,tokenAppearTimes,ifNeedOutCycle,st,scopeNode,"array","value",lineOrder);
            result += " " + temResult;
        }//使用指针变量
        else if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.pointerValue))) {
        	String temResult=generateIdentifierByType(str,index,tokenAppearTimes,ifNeedOutCycle,st,scopeNode,"pointer","value",lineOrder);
            result += " " + temResult;
        }else if(ConfigureInfo.ifRustCode&&str!=""&&str.trim().equalsIgnoreCase(RustConfigureInfo.arrayVarName)){//引用数组变量标识符
            String temResult=generateIdentifierByType(str, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,"array","self",lineOrder);
            result =temResult;

        }else if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.pointerVarName))){//引用指针变量标识符
            String temResult=generateIdentifierByType(str, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,"pointer","self",lineOrder);
            result =temResult;

        }else if(ConfigureInfo.ifRustCode&&str!=""&&(RustConfigureInfo.getVarnames().contains(str.trim()))){
            result=generateIdentifierByType(str, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,"var","-",lineOrder);
            return result;
        }//生成一个块里面的表达式
        else if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.ExpressionInBlock))){//表达式
            result=generateExpressionInBlock(str, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,lineOrder);
            return result;
        }
        return result;
    }
     ...
     * @return
     */
    public List<String> generatePrgramsOfSpecialPartForRust(String str, int index, Map<String, Integer> tokenAppearTimes, boolean ifNeedOutCycle, SymbolTable st, ScopeTree scopeNode,int lineOrder) {
        ...
        //main方法相关
        else if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.mainContent))){
            result=generateMain(str, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,lineOrder);
            //更新main函数对应的作用域到符号表
            st.setMainScope(scopeNode.getFather());
        }
        ...
        //模板函数相关
        else if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.templateFunDeclaration))){
            resultList=generateTemplateFuns(str, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,lineOrder);
            return resultList;
        }

        //类声明相关
        else if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.classDeclaration)||str.trim().equalsIgnoreCase(RustConfigureInfo.structDeclaration))){
            result=generateClassDeclaration(str, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,lineOrder);
        }

        //引用标识符相关变量
        //是否是C声明token,如果是，要更新直接包含的Identifier到符号表
        else if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.fieldDeclaration)||str.trim().equalsIgnoreCase(RustConfigureInfo.localDeclaration))){//不区分大小写，如果是声明变量的文法串
            //声明文法特殊处理，记录类型，标识符，作用域等信息
            result=generateDeclaration(str,st,scopeNode,index,ifNeedOutCycle,tokenAppearTimes,lineOrder);
        }else if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.fieldArrayDeclaration)||str.trim().equalsIgnoreCase(RustConfigureInfo.localArrayDeclaration))){//不区分大小写，如果是声明数组变量的文法串
            //声明文法特殊处理，记录类型，标识符，作用域等信息
        ...
        else if(ConfigureInfo.ifRustCode&&str!=""&&(str.trim().equalsIgnoreCase(RustConfigureInfo.ExpressionInBlock))){//块中的表达式
            resultList=generateExpressionsInBlock(str, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,lineOrder);
            return resultList;
        }
    ...
    private String generateTemplateFun(String str, int index, Map<String, Integer> tokenAppearTimes,
    	...
    	String result=generateMethodDeclaration(RustConfigureInfo.methodDeclaration, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,lineOrder,true);
		return result;
	}
    ...
    private List<String> generateTemplateFuns(String str, int index, Map<String, Integer> tokenAppearTimes,
        ...
        List<String> result=generateMethodDeclarations(RustConfigureInfo.methodDeclaration, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,lineOrder,true);
        return result;
    }
    ...
    public String generateIdentifierByTypeC(String strCopy, int index, Map<String, Integer> tokenAppearTimes, boolean ifNeedOutCycle, SymbolTable st, ScopeTree scopeNode,String identifierCategory,String object,int lineOrder) {
        ...
        }else if(ConfigureInfo.ifRustCode){
            typeType= RustConfigureInfo.varnamesMap.get(tokenType);
...
//        }
    ...
    public SymbolRecord generateDeclarationByTypeC(String typeType, SymbolTable st, ScopeTree scopeNode,int index,Map<String,Integer> tempTokenAppearTimes,boolean ifNeedOutCycle,int lineOrder) {
        SymbolRecord sr=generateDeclarationByTypeInCurScopeC(typeType, st, scopeNode, index,tempTokenAppearTimes, ifNeedOutCycle, RustConfigureInfo.rustBaseTypeLiteral,lineOrder);
...
//        if(type==null)//System.out.println("what is this"+typeType);
    ...
    /**
     * 2025.5.13
     * 生成一个块里面的表达式，用于插入算子的插入
     */
    private String generateExpressionInBlock(String str, int index, Map<String, Integer> tokenAppearTimes, boolean ifNeedOutCycle, SymbolTable st, ScopeTree scopeNode,int lineOrder) {
        if(ConfigureInfo.selectedMutationStrategy==ConfigureInfo.mutationStrategyMain2Main) {
            ...
            if (num > 50) {//一半概率用变异的函数，一半概率基于文法生成函数
                ...
                MutationRust mut = new MutationRust(ConfigureInfo.selectedMutationStrategy);
                fragment= mut.mutateIf(st);//返回插入的函数文本 并且更新函数的符号表
                result=fragment;
                return result;
            }
        }
     ...
     * qurong
     * 2025-5-13
     * 生成一个在块里面的表达式，主要用于插入变异算子
     */
    private List<String> generateExpressionsInBlock(String str, int index, Map<String, Integer> tokenAppearTimes, boolean ifNeedOutCycle, SymbolTable st, ScopeTree scopeNode,int lineOrder) {
        List<String> resultList=new ArrayList<>();
        if(ConfigureInfo.selectedMutationStrategy==ConfigureInfo.mutationStrategyMain2Main) {
            Random random = new Random();
            int num = random.nextInt(100);
            if (num > 50) {//一半概率用变异的函数，一半概率基于文法生成函数
                ...
                scopeNode = grammarGenerateService.updateScope(st, scopeNode);//方法定义也开启一个作用域，这是为了把方法的参数也能包含进作用域
                MutationRust mut = new MutationRust(ConfigureInfo.selectedMutationStrategy);
                fragment= mut.mutateIf(st);//返回插入的if块文本 并且更新函数的符号表
                ...
                if(type1!=null) {
                    switch (type1.getTypeName()) {
                        case "int[]": {
                            ...
                            paraStatement = "int" + " " + symbolRecord.getName() + "[" + RustConfigureInfo.arrayLength + "] = " + symbolRecord.getValue() + ";";
                            break;
                        }
                        case "int*": {
...
//                scopeNode = grammarGenerateService.updateScope(st, scopeNode);//方法定义也开启一个作用域，这是为了把方法的参数也能包含进作用域
//                MutationRust mut = new MutationRust(ConfigureInfo.selectedMutationStrategy);
//                fragment= mut.mutate(st);//返回插入的函数文本 并且更新函数的符号表
//                result=fragment;
//                return result;
//            }
//        }
...
//                            String ident1=paraInfo[0].trim();
                            String ident1=paraInfo[0].trim().split(" ")[1];//mut name
                            ...
                            if(leftBracket.equals("[")&&rightBracket.equals("]")) {
	                            ...
	                            sr1.setArrayLength(RustConfigureInfo.arrayLength);
	                            parameterIdentifiers.add(sr1);
	                            grammarGenerateService.updateSymTableByVar(st,sr1);//参数列表中的形参也是函数体的作用域内可以使用的变量，更新到符号表
                            }
...
//                	//System.out.println("function returntype is null");
                ...
                if(temResult.trim().equals("")){
                    ...
                    for(int j=0;j<parameterArr.length;j++){
                        String param=parameterArr[j].trim();
                        String[] paraInfo=param.split(":");//用冒号来分割类型和标识符
                        if(paraInfo.length<2){//一个参数 但是类型和标识符有问题
//                            //System.out.println("paraInfo.length less than 2: "+paraInfo.length);
                        }else if(paraInfo.length==2){//一个普通参数 类型和标识符匹配
                            String ident1=paraInfo[0].trim().split(" ")[1];
                            String type1name=paraInfo[1].trim();
                            Type type1=st.getTypeMap().get(type1name);
                            parameterList.add(type1);
                            SymbolRecord sr1=new SymbolRecord(ident1,type1,ident1,"var",scopeNode,"");
...
//        st.setFunctionSymbolRecords(functions);
     ...
     * @return
     */
    public String generateSpecialItemForRust(String item, int index, Map<String, Integer> tokenAppearTimes, boolean ifNeedOutCycle, SymbolTable st, ScopeTree scopeNode,int lineOrder) {
        String result="";
        Constraint constraint=new Constraint();
        if(ConfigureInfo.ifRustCode&&item!=""&&item.trim().equalsIgnoreCase(RustConfigureInfo.arrayVarName)){//引用数组变量标识符
            String temResult=generateIdentifierByType(item, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,"array","self",lineOrder);
            result =temResult;

        }else if(ConfigureInfo.ifRustCode&&item!=""&&(item.trim().equalsIgnoreCase(RustConfigureInfo.pointerVarName))){//引用指针变量标识符
            String temResult=generateIdentifierByType(item, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,"pointer","self",lineOrder);
            result =temResult;

        }
        
        else if(ConfigureInfo.ifRustCode&&item!=""&&(RustConfigureInfo.getVarnames().contains(item))){//引用变量标识符
            String temResult=generateIdentifierByType(item, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,"var","-",lineOrder);
            result =temResult;

        }
      //类声明相关
        else if(ConfigureInfo.ifRustCode&&item!=""&&(item.trim().equalsIgnoreCase(RustConfigureInfo.classDeclaration)||item.trim().equalsIgnoreCase(RustConfigureInfo.structDeclaration))){
            result=generateClassDeclaration(item, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,lineOrder);
            return result;
        }
        
      //模板函数相关
        if(ConfigureInfo.ifRustCode&&item!=""&&(item.trim().equalsIgnoreCase(RustConfigureInfo.templateFunDeclaration))){
            result=generateTemplateFun(item, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,lineOrder);
            return result;
        }
        
        //main方法相关
        else if(ConfigureInfo.ifRustCode&&item!=""&&(item.trim().equalsIgnoreCase(RustConfigureInfo.mainContent))){
            result=generateMain(item, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,lineOrder);
            st.setMainScope(scopeNode.getFather());
            return result;
        }//变量相关
        else if(ConfigureInfo.ifRustCode&&item!=""&&(item.trim().equalsIgnoreCase(RustConfigureInfo.fieldDeclaration)||item.trim().equalsIgnoreCase(RustConfigureInfo.localDeclaration))){//声明文法特殊处理，记录类型，标识符，作用域等信息
            String temResult=generateDeclaration(item,st,scopeNode,index,ifNeedOutCycle,tokenAppearTimes,lineOrder);
            result += " " + temResult;
        }//数组变量相关
        else if(ConfigureInfo.ifRustCode&&item!=""&&(item.trim().equalsIgnoreCase(RustConfigureInfo.fieldArrayDeclaration)||item.trim().equalsIgnoreCase(RustConfigureInfo.localArrayDeclaration))){//声明数组文法特殊处理，记录类型，标识符，数组长度，作用域等信息
            String temResult=generateArrayDeclaration(item,st,scopeNode,index,ifNeedOutCycle,tokenAppearTimes,lineOrder);
            result += " " + temResult;
        }//指针变量相关
        else if(ConfigureInfo.ifRustCode&&item!=""&&(item.trim().equalsIgnoreCase(RustConfigureInfo.fieldPointerDeclaration)||item.trim().equalsIgnoreCase(RustConfigureInfo.localPointerDeclaration))){//声明数组文法特殊处理，记录类型，标识符，作用域等信息
            String temResult=generatePointerDeclaration(item,st,scopeNode,index,ifNeedOutCycle,tokenAppearTimes,lineOrder);
            result += " " + temResult;
        }//函数相关
        else if(ConfigureInfo.ifRustCode&&item!=""&&(item.trim().equalsIgnoreCase(RustConfigureInfo.methodDeclaration))){//声明方法
            String temResult=generateMethodDeclaration(item,index,tokenAppearTimes,ifNeedOutCycle,st,scopeNode,lineOrder,false);
            result += " " + temResult;
        }//使用数组变量
        else if(ConfigureInfo.ifRustCode&&item!=""&&(item.trim().equalsIgnoreCase(RustConfigureInfo.arrayElement))) {
        	String temResult=generateIdentifierByType(item,index,tokenAppearTimes,ifNeedOutCycle,st,scopeNode,"array","value",lineOrder);
            result += " " + temResult;
        }//使用指针变量
        else if(ConfigureInfo.ifRustCode&&item!=""&&(item.trim().equalsIgnoreCase(RustConfigureInfo.pointerValue))) {
        ...
        else if(ConfigureInfo.ifRustCode&&item!=""&&(RustConfigureInfo.divisorGrammars.contains(item))){
        ...
        else if(ConfigureInfo.ifRustCode&&item!=""&&(item.trim().equalsIgnoreCase(RustConfigureInfo.ExpressionInBlock))){//块中的表达式
        	String temResult=generateExpressionInBlock(item,index,tokenAppearTimes,ifNeedOutCycle,st,scopeNode,lineOrder);
            result += " " + temResult;
        }
        return result;
    }
     ...
     * @return
     */
    public List<String> generateProgramsOfSpecialItemForRust(String item, int index, Map<String, Integer> tokenAppearTimes, boolean ifNeedOutCycle, SymbolTable st, ScopeTree scopeNode,int lineOrder) {
        List<String> resultList=new ArrayList<>();
        String result="";
        ...
        //main方法相关
            ...
            List<String> temResultList=generateExpressionsInBlock(item,index,tokenAppearTimes,ifNeedOutCycle,st,scopeNode,lineOrder);
            resultList=stringTools.combineStrList(resultList,temResultList);
            return resultList;
        }
        resultList.add(result);
    ...
    public String generateMain(String str, int index, Map<String, Integer> tokenAppearTimes, boolean ifNeedOutCycle, SymbolTable st, ScopeTree scopeNode,int lineOrder) {
        ...
        result+="unsafe {"+generateObjectCallMethod(index, tokenAppearTimes, ifNeedOutCycle, st, scopeNode,addedDeclaration,lineOrder)+"}";
...
//        }
    ...
    public String generateDeclaration(String str, SymbolTable st, ScopeTree scopeNode, int index, boolean ifNeedOutCycle, Map<String, Integer> tempTokenAppearTimes,int lineOrder) {
        ...
        if(typeType.equals("int")){
            typeType="i32";
        }else if(typeType.equals("short")){
            typeType="i16";
        }else if(typeType.equals("long")){
            typeType="i64";
        }
        //确定声明的变量的标识
        SymbolRecord symbolRecord;
        if(str.equals(RustConfigureInfo.fieldDeclaration)){//和全局变量声明文法相关 只能使用常量为其赋值
            symbolRecord=generateDeclarationByTypeInCurScope(typeType,st,scopeNode,index,tempTokenAppearTimes,ifNeedOutCycle, RustConfigureInfo.rustBaseTypeLiteral,lineOrder);
            result ="static mut "+ symbolRecord.getName()+" : "+ typeType+ "="+ symbolRecord.getValue()+ ";";
        }else{//和局部变量声明文法相关
            symbolRecord=generateDeclarationByTypeInCurScope(typeType,st,scopeNode,index,tempTokenAppearTimes,ifNeedOutCycle, RustConfigureInfo.rustBaseTypeExp,lineOrder);
            result ="let "+ symbolRecord.getName()+" : "+ typeType+ "="+ symbolRecord.getValue();
        }
    ...
    public String generateArrayDeclaration(String str, SymbolTable st, ScopeTree scopeNode, int index, boolean ifNeedOutCycle, Map<String, Integer> tempTokenAppearTimes,int lineOrder) {
        ...
        //确定声明的变量的标识
        SymbolRecord symbolRecord;
        if(str.equals(RustConfigureInfo.fieldArrayDeclaration)){//和全局变量声明文法相关 只能使用常量为其赋值
            symbolRecord=generateArrayDeclarationByTypeInCurScope(typeType,st,scopeNode,index,tempTokenAppearTimes,ifNeedOutCycle, RustConfigureInfo.rustBaseTypeLiteral,lineOrder);
            result =typeType+" "+ symbolRecord.getName()+ "["+symbolRecord.getArrayLength()+"] = "+symbolRecord.getValue()+";";
        }else{//和局部数组变量声明文法相关
            symbolRecord=generateArrayDeclarationByTypeInCurScope(typeType,st,scopeNode,index,tempTokenAppearTimes,ifNeedOutCycle, RustConfigureInfo.rustBaseTypeExp,lineOrder);
            result =typeType+" "+ symbolRecord.getName()+ "["+symbolRecord.getArrayLength()+"] = "+symbolRecord.getValue();
        }
        grammarGenerateService.updateSymTableByVar(st,symbolRecord);//更新符号表模块-定义标识符
        return result;
    ...
    public String generatePointerDeclaration(String str, SymbolTable st, ScopeTree scopeNode, int index, boolean ifNeedOutCycle, Map<String, Integer> tempTokenAppearTimes,int lineOrder) {
        ...
        //确定声明的变量的标识
        SymbolRecord symbolRecord;
        if(str.equals(RustConfigureInfo.fieldPointerDeclaration)){//和全局变量声明文法相关 只能使用常量为其赋值
            symbolRecord=generatePointerDeclarationByTypeInCurScope(typeType,st,scopeNode,index,tempTokenAppearTimes,ifNeedOutCycle, RustConfigureInfo.rustBaseTypeLiteral,lineOrder);
            result =typeType+" *"+ symbolRecord.getName()+ " = &"+symbolRecord.getValue()+";";
        }else{//和局部指针变量声明文法相关
            symbolRecord=generatePointerDeclarationByTypeInCurScope(typeType,st,scopeNode,index,tempTokenAppearTimes,ifNeedOutCycle, RustConfigureInfo.rustBaseTypeExp,lineOrder);
            result =typeType+" *"+ symbolRecord.getName()+ " = &"+symbolRecord.getValue();
        }
        grammarGenerateService.updateSymTableByVar(st,symbolRecord);//更新符号表模块-定义标识符
        return result;
    ...
    public String addReturn(String str, Type type, int index, Map<String, Integer> tokenAppearTimes, boolean ifNeedOutCycle, SymbolTable st, ScopeTree scopeNode,int lineOrder) {
        ...
        if(type.getTypeName().equals("void")){//方法的返回值是void类型
            ...
            addStr="unsafe { return "+returnContent+"}";
...
//		            //System.out.println("unknown type "+"strCopy");
	...
	public String getNewIdentifier(String typeType, SymbolTable st, ScopeTree scopeNode, int index,
		...
		switch(identifierCategory) {
			case "var": {
                ...
                defineStatement ="static mut "+ symbolRecord.getName()+" : "+ typeType+ "="+ symbolRecord.getValue()+ ";";
			...
			case "array": {
                ...
                defineStatement ="static mut "+ symbolRecord.getName()+" : i32=0;";
    ...
    private SymbolRecord generateDeclarationByType(String typeType, SymbolTable st, ScopeTree scopeNode,int index,Map<String,Integer> tempTokenAppearTimes,boolean ifNeedOutCycle,int lineOrder) {
        SymbolRecord sr=generateDeclarationByTypeInCurScope(typeType, st, scopeNode, index,tempTokenAppearTimes, ifNeedOutCycle, RustConfigureInfo.rustBaseTypeLiteral,lineOrder);
//        sr.setScope(scopeNode.getLocateType().getScopeNode());//用当前作用域所在类的作用域
        sr.setScope(scopeNode.getRoot());//用根作用域 生成大量全局变量开关2
        return sr;
    }
    ...
    private SymbolRecord generateArrayDeclarationByType(String typeType, SymbolTable st, ScopeTree scopeNode,int index,Map<String,Integer> tempTokenAppearTimes,boolean ifNeedOutCycle,int lineOrder) {
        SymbolRecord sr=generateArrayDeclarationByTypeInCurScope(typeType, st, scopeNode, index,tempTokenAppearTimes, ifNeedOutCycle, RustConfigureInfo.rustBaseTypeLiteral,lineOrder);
//        sr.setScope(scopeNode.getLocateType().getScopeNode());//用当前作用域所在类的作用域
        sr.setScope(scopeNode.getRoot());//用根作用域 生成大量全局变量开关2
        return sr;
    }
    ...
    private SymbolRecord generatePointerDeclarationByType(String typeType, SymbolTable st, ScopeTree scopeNode,int index,Map<String,Integer> tempTokenAppearTimes,boolean ifNeedOutCycle,int lineOrder) {
        SymbolRecord sr=generatePointerDeclarationByTypeInCurScope(typeType, st, scopeNode, index,tempTokenAppearTimes, ifNeedOutCycle, RustConfigureInfo.rustBaseTypeLiteral,lineOrder);
//        sr.setScope(scopeNode.getLocateType().getScopeNode());//用当前作用域所在类的作用域
        sr.setScope(scopeNode.getRoot());//用根作用域 生成大量全局变量开关2
        return sr;
    }
...
//            v54TudI<int> Zd70416;
//            let Zd70416: v54TudI<i32> = v54TudI {};
//            createObjectStr+=typename+"<"+ RustConfigureInfo.Typename+">"+" "+objectIdentifier+";";//
            createObjectStr+="let "+objectIdentifier+": "+ typename+"<"+ RustConfigureInfo.Typename+">="+typename+"{};";//
            //更新符号表
    ...
    private String generateCallsForBlobalFuns(List<FunctionSymbolRecord> functions, SymbolTable st, ScopeTree scopeNode,
        ...
        for(FunctionSymbolRecord fun:functions){
            ...
            if(returnType==null||returnType.getTypeName()==null||returnType.getTypeName().equals("void")){//方法没有返回值 或者是插入的函数算子，没有把返回值类型更新到函数的符号表上
                ...
                if(returnType!=null) {
                    switch (returnType.getTypeName()) {
                        case "int[]": {
                            symbolRecord = generateArrayDeclarationByTypeInCur("int[]", st, scopeNode, index, tokenAppearTimes, ifNeedOutCycle, lineOrder);//
                            functionCall += "int " + symbolRecord.getName() + "[" + RustConfigureInfo.arrayLength + "]";
                            break;
                        }
                        case "int*": {
                            symbolRecord = generatePointerDeclarationByTypeInCur("int*", st, scopeNode, index, tokenAppearTimes, ifNeedOutCycle, lineOrder);//
            ...
            if(fun.isTemplateFun()) {
            	functionCall+="<"+ RustConfigureInfo.Typename+">";
        	}
                ...
                for (Type type1 : parameterList) {
                    ...
                    if(type1!=null) {
                        switch (type1.getTypeName()) {
                            ...
                            default: {//非数组和指针变量
                                symbolRecord = generateDeclarationByTypeInCur(type1.getTypeName(), st, scopeNode, index, tokenAppearTimes, ifNeedOutCycle, lineOrder);//
                                paraStatement = symbolRecord.getName() +":"+type1.getTypeName() + " = " + symbolRecord.getValue() + ";";
                            }
                        }
                    }else{
        ...
        if(returnType==null||returnType.getTypeName().equals("void")){//方法没有返回值
            ...
            if(returnType!=null) {
                switch (returnType.getTypeName()) {
                    case "int[]": {
                        symbolRecord = generateArrayDeclarationByTypeInCur("int[]", st, scopeNode, index, tokenAppearTimes, ifNeedOutCycle, lineOrder);//
                        lamdaCall += "int " + symbolRecord.getName() + "[" + RustConfigureInfo.arrayLength + "]";
                        break;
                    }
                    case "int*": {
                        symbolRecord = generatePointerDeclarationByTypeInCur("int*", st, scopeNode, index, tokenAppearTimes, ifNeedOutCycle, lineOrder);//
    ...
    public SymbolRecord generateArrayDeclarationByTypeInCurScope(String typeType, SymbolTable st, ScopeTree scopeNode, int index, Map<String, Integer> tempTokenAppearTimes, boolean ifNeedOutCycle, Map<String, String> typeExpression,int lineOrder) {
        ...
        String identifier="";
        String identifierValue="{";
        int arrayLength=(int)(Math.random()* RustConfigureInfo.arrayLength+1);
        Constraint constraint=new Constraint();
//        Constraint constraint=new Constraint();
        identifier=grammarGenerateService.getIdentifier(index, tempTokenAppearTimes, ifNeedOutCycle, st, scopeNode,lineOrder);
...
//        if(type==null)//System.out.println("what is this"+typeType);
    ...
    public SymbolRecord generatePointerDeclarationByTypeInCurScope(String typeType, SymbolTable st, ScopeTree scopeNode, int index, Map<String, Integer> tempTokenAppearTimes, boolean ifNeedOutCycle, Map<String, String> typeExpression,int lineOrder) {
        ...
        varIdentifier=generateIdentifierByTypeC("i32", index,tempTokenAppearTimes, ifNeedOutCycle, st, scopeNode.getRoot(),"var","-",lineOrder);
//        }
    	...
    	switch(categpory){
	    	case "var":{
	            switch(typeName){
	                ...
	                case "bool":
	                case "long":
	                default :printString="println!(\"{}\","+baseIden.getName()+");";break;
	            }
	    	...
	    	case "array":{
	    		switch(typeName){
		            ...
		            case "bool[]":
		            case "long[]":
		            default :printString="for(int i=0;i<"+baseIden.getArrayLength()+";i++){println!(\"{}\","+baseIden.getName()+"[i]);}";break;
	    		}
    	...
    	switch(categpory){
	    	case "var":{
	            switch(typeName){
	                ...
	                case "bool":
	                case "long":
	                default :printString="println!(\"{}\", "+objName+"."+baseIden.getName()+");";break;
	            }
	            printPositionOfSymbolRecord.put(printIndex, baseIden);
	            printStrings.put(printIndex, printString);
	            break;
	    	...
	    	case "array":{
	    		switch(typeName){
		            ...
		            case "bool[]":
		            case "long[]":
		            default :printString="for(int i=0;i<"+baseIden.getArrayLength()+";i++){println!(\"{}\", "+objName+"."+baseIden.getName()+"[i]);}";break;
	    		}
	    		for(int i=0;i<baseIden.getArrayLength();i++) {
	    			printPositionOfSymbolRecord.put(printIndex, baseIden);
	    			printStrings.put(printIndex, printString);
...
//	return result;
    ...
    public String insertBreakToLoop(String loopSegment, String identifierCount) {
        ...
        breakString+= RustConfigureInfo.loopBreak+";}";
