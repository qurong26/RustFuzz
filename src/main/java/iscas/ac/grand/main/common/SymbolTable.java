...
public class SymbolTable implements Serializable {
    ...
    public void initailTypeMap(List<String> baseTypes) {
        ...
        if(ConfigureInfo.ifJavaCode) {
            ...
            for (int i = 0; i < baseTypes.size(); i++) {
                Type tempType = new Type(i, baseTypes.get(i), RustConfigureInfo.rustBaseTypeLiteral.get(baseTypes.get(i)), RustConfigureInfo.rustBaseTypeExp.get(baseTypes.get(i)));//类型
                this.typeMap.put(baseTypes.get(i), tempType);
            }
