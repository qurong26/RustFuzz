...
import iscas.ac.grand.main.antlr4.cplus.ASTWalkFromTestSuit;
import iscas.ac.grand.main.antlr4.cplus.CPP14Parser;
import iscas.ac.grand.main.antlr4.cplus.CPPParserASTListener;
import iscas.ac.grand.main.antlr4.cplus.CustomRuleContextDecorator;
...
import iscas.ac.grand.mutate.rust.GrammarGenerateForRust;
import iscas.ac.grand.mutate.rust.RustConfigureInfo;
...
//            CppForInsert cfi=new CppForInsert("F:"+File.separator+"data-compiler-testing"+File.separator+"2024-12-16cpp-gcc-llvm-testsuit"+File.separator+"gcc-testsuit-slice"+File.separator+"1225"+File.separator+"cppProgramList0514.txt");//静态的程序列表
//            CppForInsert cfi=new CppForInsert("F:"+File.separator+"data-compiler-testing"+File.separator+"2024-12-16cpp-gcc-llvm-testsuit"+File.separator+"programs-deleted-failed-slice"+File.separator+"0120"+File.separator+"cppProgramList0514.txt");//静态的程序列表
            CppForInsert cfi = new CppForInsert("main2main" + File.separator + "cppProgramList0625.txt");//静态的程序列表 用于main-main策略，基于匹配切片而非AST
        }
...
//        	int i=number%10000;//远程执行相关
            ...
            st.initailTypeMap(baseTypes);//初始化类型映射
            String program = "";
...
//                    }
//                }else
                if (ConfigureInfo.ifRustCode) {//将void替换为() 将int、short、long分别替换为i32、i16、i64
                    //programAuto=autoProgram(program);
//                    programAuto=program;
                    program = replaceVoid(program);
                    program = replaceIntShortLong(program);
                }
...
//        remoteAutoTest.disRemoteAutoTest();
     ...
     * qurong
     * 2025-5-21
     * 将int、short、long分别替换为i32、i16、i64
     *
     * @param program
     * @return
     */
    private String replaceIntShortLong(String program) {
     ...
     * @return
     */
    private String replaceVoid(String program) {
        String result = program;
        ...
        if (program != "") {
            ...
            if (ConfigureInfo.isFangzhouMulti) {//如果是为方舟编译器生成批量的测试程序  SANITY000206-cprogram206
                if (ConfigureInfo.ifJavaCode)
                    programPath = grammars.get(index).getPath().replace(".g4", "" + File.separator + "samll_5000" + File.separator + "" + "APP000" + i + "-" + programName + File.separator);//保存测试的目录
                if (ConfigureInfo.ifCCode)
                    programPath = grammars.get(index).getPath().replace(".g4", "" + File.separator + "small_5000" + File.separator + "" + "SANITY000" + i + "-cprogram" + i + File.separator);//保存测试的目录
...
//                    programPath=ConfigureInfo.outPutdirectory+File.separator+j+File.separator+"programs"+File.separator;
    ...
    private String postProcessAfterGeneration(String program, SymbolTable st) {
        ...
        if (ConfigureInfo.ifRustCode) {
            program = insertAddDeclarationToHeadRust(st, program);
//            program=insertAddIncludeRust(program,st);
        ...
        if (ConfigureInfo.ifPythonCode && PythonConfigureInfo.ifFormatJPythonCode) {
            ...
            System.out.println(program);
//            program = getProtectedProgram(program);
//            program=getProtectedProgram2(program);
            program = getProtectedProgram3(program);
            System.out.println(program);
        } else {
    ...
    private String getProtectedProgram3(String program) {
        // 1. 保护字符串
        Map<String, String> strMap = new HashMap<>();
        Pattern strPattern = Pattern.compile("(\"(?:\\\\\"|[^\"])*?\")|(r#+\".*?\"#+)");
        Matcher m = strPattern.matcher(program);
        StringBuffer sb = new StringBuffer();
        int counter = 0;

        while (m.find()) {
            String placeholder = "___STR_" + (counter++) + "___";
            strMap.put(placeholder, m.group());
            m.appendReplacement(sb, placeholder);
        }
        m.appendTail(sb);
        String protectedProgram =sb.toString();

// 2. 应用换行规则
        protectedProgram =protectedProgram.replaceAll("(?<!(else|try|catch|finally))\\}", "}\n")
                .replaceAll(";(?!(\\s*[);,]))", ";\n");

// 3. 恢复字符串
        for (Map.Entry<String, String> e : strMap.entrySet()) {
            protectedProgram =protectedProgram.replace(e.getKey(), e.getValue());
        }

// 4. 关键修复（新增）
        protectedProgram =protectedProgram.replaceAll("println!\\(\"\\{\\\\n\"\\)", "println!(\"{}\")")
                .replaceAll("println!\\(\"\\\\n\\}\"\\)", "println!(\"{}\")")
                .replaceAll("\\?\\s*\\n\\s*\\{", "? {")
                .replaceAll("\\}\\s*\\n\\s*else\\s*\\{", "} else {")
                .replaceAll("if\\s*\\(\\s*\\n", "if (");
    ...
    private String getProtectedProgram2(String program) {
            // 1. 保护字符串
            Map<String, String> strMap = new HashMap<>();
            Pattern strPattern = Pattern.compile("(\"(?:\\\\\"|[^\"])*?\")|(r#+\".*?\"#+)");
            Matcher m = strPattern.matcher(program);
            ...
            // 2. 预处理三元表达式 - 关键改进
            // 添加空格确保正确解析
            protectedProgram = protectedProgram
                    .replaceAll("ifif", "if if")
                    .replaceAll("if([a-zA-Z_])", "if $1") // 在if和标识符间加空格
                    .replaceAll("([!=]==)([^\\s])", "$1 $2") // 运算符周围加空格
                    .replaceAll("\\?\\{", "? {")
            ...
            // 3. 应用换行规则
            protectedProgram = protectedProgram
                    .replaceAll("(?<!(else|try|catch|finally))\\}", "}\n")
                    .replaceAll(";(?!(\\s*[);,]))", ";\n");
            ...
            Pattern ternaryPattern = Pattern.compile(
                    "(if if\\s*\\w+\\s*==\\s*0\\s*\\?\\s*\\{.*?\\}\\s*else\\s*\\{.*?\\})" +
                            "\\s*==\\s*0\\s*\\?\\s*\\{.*?\\}\\s*else\\s*\\{.*?\\}"
            );

            Matcher ternaryMatcher = ternaryPattern.matcher(protectedProgram);
            StringBuffer fixedBuffer = new StringBuffer();

            while (ternaryMatcher.find()) {
                String ternaryExpr = ternaryMatcher.group();
                // 格式化嵌套三元表达式
                String formatted = ternaryExpr
                        .replaceAll("\\?\\s*\\{", "? {")
                        .replaceAll("\\}\\s*else\\s*\\{", "} else {")
                        .replaceAll("==0\\s*\\?", "== 0 ?")
                        .replaceAll("\\}\\s*([%&|])", "} $1");

                ternaryMatcher.appendReplacement(fixedBuffer, formatted);
            }
            ternaryMatcher.appendTail(fixedBuffer);
            protectedProgram = fixedBuffer.toString();
     ...
     * @return
     */
    private String getProtectedProgram(String program) {
        // 1. 保护字符串
        Map<String, String> strMap = new HashMap<>();
        Pattern strPattern = Pattern.compile("(\"(?:\\\\\"|[^\"])*?\")|(r#+\".*?\"#+)");
        Matcher m = strPattern.matcher(program);
...
// 4. 关键修复（新增）
        ...
        protectedProgram = protectedProgram
                // 修复三元表达式中嵌套的 if 条件
                .replaceAll("ifif", "if if")  // 确保 if if 之间有空格
                .replaceAll("\\?\\s*\\{", "? {")  // 确保 ? 和 { 之间没有换行
                .replaceAll("else\\s*\\{", "else {")  // 确保 else 和 { 之间没有换行
                .replaceAll("==0\\s*\\?", "== 0 ?")  // 标准化条件格式
                .replaceAll("\\}\\s*\\n\\s*%", "} %")  // 修复运算符连接问题
                .replaceAll("\\}\\s*else\\s*\\{", "} else {");  // 确保 else 正确连接
        return protectedProgram;
    }
...
//        }
        ...
        if(ifScopeBodyGrammar(strCopy)){
        	...
        	if(ConfigureInfo.ifCCode) {
                ...
                if(strCopy.equals(RustConfigureInfo.methodDeclarationTotal)||strCopy.equals(RustConfigureInfo.methodDeclaration)) {//把作用域和函数关联起来
                    if(st.getFunctionSymbolRecords()!=null&&st.getFunctionSymbolRecords().size()>0) {
                        scopeNode.setFun(st.getFunctionSymbolRecords().get(st.getFunctionSymbolRecords().size() - 1));
                    }
                }
	...
	private String addOutput(SymbolTable st, ScopeTree scopeNode,String str) {
        ...
        GrammarGenerateForRust ggrust=new GrammarGenerateForRust(grammars);
        if(ConfigureInfo.ifJavaCode){
            ...
            outPutStr=ggrust.generateOutPut(st, scopeNode);
            result=ggrust.insertStr2ScopeEnd(str,outPutStr);
        }
    ...
    private String getLoopCountIdentifier(String identifierCount,ScopeTree scopeNode) {
        ...
        if(ConfigureInfo.ifJavaCode){
            ...
            result= "let "+identifierCount+":"+RustConfigureInfo.loopCountType+"; "+identifierCount+"=0;";
            return result;
        }
    ...
    private String generateSpecialPartForItem(String item, int index, Map<String, Integer> tokenAppearTimes, boolean ifNeedOutCycle, SymbolTable st, ScopeTree scopeNode,int lineOrder) {
        ...
        if(ifJavaSpecialItem(item)){
	        ...
	        return result;
	    }else if(ifRustSpecialItem(item)){
            result=ggrust.generateSpecialItemForRust(item, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,lineOrder);
            return result;
        }
        return result;
    }
...
//        }else
        if(ifCPlusSpecialItem(item)) {
            ...
            result = ggrust.generateProgramsOfSpecialItemForRust(item, index, tokenAppearTimes, ifNeedOutCycle, st, scopeNode, lineOrder);
            return result;
        }
        return result;
    }
    ...
    private String generateIfSpecialGrammar(String str, int index, Map<String, Integer> tokenAppearTimes, boolean ifNeedOutCycle, SymbolTable st, ScopeTree scopeNode,int lineOrder) {
        ...
        if(ifJavaSpecialGrammar(str)){
	        ...
	        return result;
	    }else if(ifRustSpecialGrammar(str)){
            result=ggrust.generateSpecialPartForRust(str, index,tokenAppearTimes, ifNeedOutCycle, st, scopeNode,lineOrder);
            return result;
        }
        return result;
    }
    ...
    private String getTabProfixByDepth(String stmt,ScopeTree scopeNode, String str) {
        ...
        if(ConfigureInfo.ifJavaCode){
            ...
            GrammarGenerateForRust ggf=new GrammarGenerateForRust();
            return ggf.getTabProfixByDepth(scopeNode)+str;
        }else if(ConfigureInfo.ifPythonCode){
    ...
    private boolean isMethod(String stmt) {
        if(ConfigureInfo.ifJavaCode&&stmt!=""&&(JavaConfigureInfo.statementGrammars.contains(stmt))){
            ...
            return true;
        }else if(ConfigureInfo.ifRustCode&&stmt!=""&&(RustConfigureInfo.statementGrammars.contains(stmt))){
            return true;
     ...
     * qurong
     * 2025-5-9
     * 是否为Rust中的特殊文法部分，例如声明方法、类型、变量的文法
     * @param str
     * @return
     */
    private boolean ifRustSpecialGrammar(String str) {
        if(str==null){
            //System.out.println("ah c++ str is null!");
            return false;
        }
        if(ConfigureInfo.ifRustCode&&str!="") {
            switch(str.trim()){
                case RustConfigureInfo.methodDeclaration:;
                case RustConfigureInfo.ExpressionInBlock:;
                case RustConfigureInfo.mainContent:;
                case RustConfigureInfo.classDeclaration:;
                case RustConfigureInfo.structDeclaration:;
                case RustConfigureInfo.templateFunDeclaration:;

                case RustConfigureInfo.fieldDeclaration:;
                case RustConfigureInfo.localDeclaration:;
                case RustConfigureInfo.fieldArrayDeclaration:;
                case RustConfigureInfo.localArrayDeclaration:;
                case RustConfigureInfo.fieldPointerDeclaration:;
                case RustConfigureInfo.arrayElement:;
                case RustConfigureInfo.pointerValue:;
                case RustConfigureInfo.localPointerDeclaration:return true;
                default:
            }
            if (RustConfigureInfo.getVarnames().contains(str.trim())) {
                return true;
            }
        }
        return false;
     ...
     * @return
     */
    private boolean ifRustSpecialItem(String str) {
        if(ConfigureInfo.ifRustCode&&str!="") {
            switch(str.trim()){
                case RustConfigureInfo.methodDeclaration:;
                case RustConfigureInfo.ExpressionInBlock:;
            ...
            if (RustConfigureInfo.getVarnames().contains(str.trim())||RustConfigureInfo.divisorGrammars.contains(str.trim())) {
                return true;
            }
        }
        return false;
    ...
    private boolean ifLoopGrammar(String str) {
        if(ConfigureInfo.ifJavaCode&&str!=""&&(JavaConfigureInfo.loopGrammars.contains(str))){
            ...
            return true;
        }else if(ConfigureInfo.ifRustCode&&str!=""&&(RustConfigureInfo.loopGrammars.contains(str))){
            return true;
    ...
    private boolean ifDivisorGrammar(String str) {
        if(ConfigureInfo.ifJavaCode&&str!=""&&(JavaConfigureInfo.divisorGrammars.contains(str))){
            ...
            return true;
        }else if(ConfigureInfo.ifRustCode&&str!=""&&(RustConfigureInfo.divisorGrammars.contains(str))){
            return true;
    ...
    private boolean ifStatementGrammar(String str) {
        if(ConfigureInfo.ifJavaCode&&str!=""&&(JavaConfigureInfo.statementGrammars.contains(str))){
            ...
            return true;
        }else if(ConfigureInfo.ifRustCode&&str!=""&&(RustConfigureInfo.statementGrammars.contains(str))){
            return true;
    ...
    private boolean ifDefineOrModifyGrammar(String str) {
        if(ConfigureInfo.ifJavaCode&&str!=""&&(JavaConfigureInfo.defineOrModifyGrammars.contains(str))){
            ...
            return true;
        }else if(ConfigureInfo.ifRustCode&&str!=""&&(RustConfigureInfo.defineOrModifyGrammars.contains(str))){
            return true;
    ...
    private boolean ifScopeBodyGrammar(String str) {
        if(ConfigureInfo.ifJavaCode&&str!=""&&(JavaConfigureInfo.scopeBodyGrammars.contains(str))){
            ...
            return true;
        }else if(ConfigureInfo.ifRustCode&&str!=""&&(RustConfigureInfo.scopeBodyGrammars.contains(str))){
            return true;
    ...
    private boolean ifMainBodyGrammar(String str) {
        if(ConfigureInfo.ifJavaCode&&str!=""&&(JavaConfigureInfo.mainMethodDeclaration.equals(str))){
            ...
            return true;
        }else if(ConfigureInfo.ifRustCode&&str!=""&&(RustConfigureInfo.mainMethodDeclaration.equals(str))){
            return true;
    ...
    private boolean ifMethodGrammar(String str) {
        if(ConfigureInfo.ifJavaCode&&str!=""&&(str.equals(JavaConfigureInfo.methodDeclaration))){
            ...
            return true;
        }else if(ConfigureInfo.ifRustCode&&str!=""&&(str.equals(RustConfigureInfo.methodDeclarationTotal)||str.equals(RustConfigureInfo.methodDeclaration))){
            return true;
    ...
    private boolean ifBlockGrammar(String str) {
        if(ConfigureInfo.ifJavaCode&&str!=""&&(str.equals(JavaConfigureInfo.blockDeclaration))){
            ...
            return true;
        }else if(ConfigureInfo.ifRustCode&&str!=""&&(str.equals(RustConfigureInfo.blockDeclaration))){
            return true;
    ...
    private boolean ifScopeGrammar(String str) {
        if(ConfigureInfo.ifJavaCode&&str!=""&&(JavaConfigureInfo.scopeGrammars.contains(str))){
            ...
            return true;
        }else if(ConfigureInfo.ifRustCode&&str!=""&&(RustConfigureInfo.scopeGrammars.contains(str))){
            return true;
    ...
    private boolean ifVarnameGrammar(String str) {
        if(ConfigureInfo.ifJavaCode&&str!=""&&(JavaConfigureInfo.getVarnames().contains(str))){
            ...
            return true;
        }else if(ConfigureInfo.ifRustCode&&str!=""&&(RustConfigureInfo.getVarnames().contains(str))){
            return true;
    ...
    public String generateVarname(String strCopy, int index, Map<String, Integer> tokenAppearTimes, boolean ifNeedOutCycle, SymbolTable st, ScopeTree scopeNode,int lineOrder) {
        if(ConfigureInfo.ifJavaCode){
            ...
            GrammarGenerateForRust ggf=new GrammarGenerateForRust();
            return ggf.generateIdentifierByTypeC( strCopy,  index, tokenAppearTimes,  ifNeedOutCycle,  st,  scopeNode, RustConfigureInfo.varnameCategoryMap.get(strCopy),"self",lineOrder);
        }else if(ConfigureInfo.ifPythonCode){
            PythonGrammarGenerateService pgg=new PythonGrammarGenerateService();
    ...
    public List<String> generateVarnames(String strCopy, int index, Map<String, Integer> tokenAppearTimes, boolean ifNeedOutCycle, SymbolTable st, ScopeTree scopeNode,int lineOrder) {
        ...
        if(ConfigureInfo.ifJavaCode){
            ...
            GrammarGenerateForRust ggf=new GrammarGenerateForRust();
            result=ggf.generateIdentifiersByTypeC( strCopy,  index, tokenAppearTimes,  ifNeedOutCycle,  st,  scopeNode, RustConfigureInfo.varnameCategoryMap.get(strCopy),"self",lineOrder);
        }else if(ConfigureInfo.ifPythonCode){
            PythonGrammarGenerateService pgg=new PythonGrammarGenerateService();
...
//        st.setTypeMap(typeMap);
    ...
    public String getIdentifier(int index, Map<String, Integer> tempTokenAppearTimes, boolean ifNeedOutCycle, SymbolTable st, ScopeTree scopeNode,int lineOrder) {
        ...
        if(ConfigureInfo.ifCCode) {
            ...
            tempItem= RustConfigureInfo.identifierToken;
    ...
    private boolean checkIdentifierValid(String identifier, List<String> identifierNameList) {
        ...
        if(JavaConfigureInfo.getKeyWords().contains(identifier)){
            ...
            return result;
        }else if(RustConfigureInfo.getKeyWords().contains(identifier)){
            return result;
        ...
        if(ConfigureInfo.ifCCode) {
            ...
            for(String keyword:RustConfigureInfo.getKeyWords()) {
                if(identifier.contains(keyword)) {
                    return result;
                }
            }
...
//        result=declarationString+program;
    ...
    /**
     * 2025-5-9
     * 在生成rust程序中，一些标识符未声明，记录在临时列表里，在程序生成结束后，一次性的添加到程序最开始
     * @param st
     * @param program
     * @return
     */
    private String insertAddDeclarationToHeadRust(SymbolTable st, String program) {
        List<String> addedDeclaration=st.getAddedDeclaration().get(st.getProgramName());
        if(addedDeclaration==null||addedDeclaration.size()==0){
            if(ConfigureInfo.ifRustCode) {
                program=program.replaceAll(RustConfigureInfo.insertPositionVar, " ");
//            	program=program.replaceAll(RustConfigureInfo.insertPositionPointer, " ");
            }
            return program;
        }
        String result="";
...
//            }else
        ...
        if(ConfigureInfo.ifRustCode) {
            String head=program.substring(0,program.indexOf(RustConfigureInfo.insertPositionVar));
//            String middle=program.substring(program.indexOf(RustConfigureInfo.insertPositionVar),program.indexOf(CConfigureInfo.insertPositionPointer));
            String tail=program.substring(program.indexOf(RustConfigureInfo.insertPositionVar));
//            result=head+declarationStringVar+middle+declarationStringPointer+tail;
            result=head+declarationStringVar+tail;
            result= result.replaceAll(RustConfigureInfo.insertPositionVar, " ");
//            result=result.replaceAll(RustConfigureInfo.insertPositionPointer, " ");
        }
//        result=declarationString+program;
        return result;
...
//        includeString+="#include <stdio.h> \r\n";
    ...
    /**
     * 2025-5-9
     * 在生成Rust程序中的顶部，加入include头文件
     * @param program
     * @return
     */
    private String insertAddIncludeRust(String program,SymbolTable st) {
        String result;
        String includeString="";
        includeString+="#include <iostream> \r\n";//c++
        if(ConfigureInfo.mutationAvailable) {//程序变异相关
