 ...
 * 包含各种变异算子
 */
public class MutationRust {
    private String mutationStrategy="no mutation";
    public MutationRust(String selectedMutationStrategy) {
    ...
    public CppProgram mutationMain2Main(){
        ...
        if(ConfigureInfo.mutationStrategies.contains(ConfigureInfo.selectedMutationStrategy)&&ConfigureInfo.selectedMutationStrategy.equals(ConfigureInfo.mutationStrategyMain2Main)) {//main-main变异
            ...
            cppList=deleteIfWhileForEmpty(cppList);
     ...
     * qurong
     * 2025-5-9
     * 删除程序列表中if/for/while内容都为空的程序
     * @param rustList
     * @return
     */
    private List<CppProgram> deleteIfWhileForEmpty(List<CppProgram> rustList) {
        List<CppProgram> result=new ArrayList<>();
        for(CppProgram rust:rustList){
            if(rust.getWhileList()!=null&&rust.getWhileList().size()>0){
                result.add(rust);
            }else if(rust.getForList()!=null&&rust.getForList().size()>0){
                result.add(rust);
            }else if(rust.getIfList()!=null&&rust.getIfList().size()>0){
                result.add(rust);
            }
        }
        return result;
...
//                    updateAddScopeOut(st, selectedProgram);//把关联的比如函数定义头文件等等插入
...
//        }
     ...
     * @param st
     */
    private void updateAddScopeOut(SymbolTable st,CppProgram selectedProgram) {
     ...
     * qurong
     * 2025-5-9
     * 变异方法，根据不同的变异选项来调用不同的变异方法-if变异算子的插入
     * @param st
     * @return
     */
    public String mutateIf( SymbolTable st) {
        String result="";
        if(ConfigureInfo.selectedMutationStrategy==ConfigureInfo.mutationStrategyMain2Main) {
            if (ConfigureInfo.mutationStrategies.contains(ConfigureInfo.selectedMutationStrategy)) {//main-main变异，将一个源程序main之外的部分插入到目标程序main函数之外，将源程序main之内的部分插入到目标程序main函数之内
                MutationRust mut = new MutationRust(ConfigureInfo.selectedMutationStrategy);
                CppProgram selectedProgram = mut.mutationMain2Main();
                if (selectedProgram != null && selectedProgram.getIfList() != null) {
                    String ifStat="";
                    for(ScopeBlockForSlice sbfs:selectedProgram.getIfList()){
                        ifStat+=sbfs.getScopeBodyFragment()+"\n";//每个if作用域片段
                    }
                    result = ifStat+ "\n" +"//"+selectedProgram.getCppFilePath()+"\n"+"//"+selectedProgram.getCppFileName() + "\n" ;//加入main函数内部的语句，以及该程序的名称和路径信息
                }

                if (selectedProgram != null && selectedProgram.getForList() != null) {
                    String forStat="";
                    for(ScopeBlockForSlice sbfs:selectedProgram.getForList()){
                        forStat+=sbfs.getScopeBodyFragment()+"\n";//每个for作用域片段
                    }
                    result += forStat+ "\n" +"//"+selectedProgram.getCppFilePath()+"\n"+"//"+selectedProgram.getCppFileName() + "\n" ;//加入main函数内部的语句，以及该程序的名称和路径信息

                }

                if (selectedProgram != null && selectedProgram.getWhileList() != null) {
                    String whileStat="";
                    for(ScopeBlockForSlice sbfs:selectedProgram.getWhileList()){
                        whileStat+=sbfs.getScopeBodyFragment()+"\n";//每个while作用域片段
                    }
                    result += whileStat+ "\n" +"//"+selectedProgram.getCppFilePath()+"\n"+"//"+selectedProgram.getCppFileName() + "\n" ;//加入main函数内部的语句，以及该程序的名称和路径信息

                }
                if (selectedProgram.getAssignmentList() != null) {//把程序中所有的初始化语句添加到if语句的前面
                    for(String str:selectedProgram.getAssignmentList()) {
                        result = str+ "\n" + result;
                    }
                }
//                updateAddScopeOut(st, selectedProgram);//把关联的比如函数定义头文件等等插入
...
//                System.out.println("length of function context"+funList.size());
     ...
     * @return
     */
    public List<String> mutate(SymbolTable st, ScopeTree scopeNode, boolean isTemplate, int index, Map<String, Integer> tokenAppearTimes, boolean ifNeedOutCycle, GrammarGenerateForRust ggfr) {
            ...
            if (ConfigureInfo.mutationStrategies.contains(ConfigureInfo.selectedMutationStrategy)) {//AstFun变异
                ...
                while (!funAvaiable) {
                    ...
                    if (tempFun != "0") {//如果返回值为0，表示解析该函数的部分出现异常，需要重新选择一个函数
                        ...
                        List<String> enumerateResult=getEnumeratorByFragment(tempFun,fsr, scopeNode, isTemplate, st,index, tokenAppearTimes, ifNeedOutCycle,ggfr,selectedFunContext);//枚举后的程序片段列表
    ...
    private List<String> getEnumeratorByFragment(String fragment, FunctionSymbolRecord fsr, ScopeTree scopeNode, boolean isTemplate,
                                                 ...
                                                 GrammarGenerateForRust ggfc, CPP14Parser.FunctionDefinitionContext selectedFunContext) {
        List<String> result=new ArrayList<>();
        ...
        ggfc=ggfc==null?new GrammarGenerateForRust():ggfc;
...
//                            //System.out.println("paraInfo.length less than 2: "+paraInfo.length);
                    ...
                    GrammarGenerateForRust.grammarGenerateService.updateSymTableByVar(st,sr1);//参数列表中的形参也是函数体的作用域内可以使用的变量，更新到符号表
...
//            String funBody=selectedFunContext.functionBody().getText();
                ...
                GrammarGenerateForRust ggfc = new GrammarGenerateForRust();
...
//                System.out.println("generateOutPut after");
        ...
        GrammarGenerateForRust.grammarGenerateService.updateSymTableByFun(st,fsr);//使用新声明的函数更新符号表模块
