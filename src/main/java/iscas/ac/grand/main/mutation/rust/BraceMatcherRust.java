package iscas.ac.grand.main.mutation.rust;
...
import iscas.ac.grand.main.mutation.*;
...
public class BraceMatcherRust {
    ...
    public static void main(String[] args) {
        BraceMatcherRust bm=new BraceMatcherRust();
        bm.sliceCpp();
//        bm.sliceFun();
...
//        String directoryPath = "F:"+File.separator+"data-compiler-testing"+File.separator+"2024-12-16cpp-gcc-llvm-testsuit"+File.separator+"gcc-testsuit";
//        String outPutPath = "F:"+File.separator+"data-compiler-testing"+File.separator+"2024-12-16cpp-gcc-llvm-testsuit"+File.separator+"gcc-testsuit-slice"+File.separator+"1225";

//        String directoryPath = "F:"+File.separator+"data-compiler-testing"+File.separator+"2024-12-16cpp-gcc-llvm-testsuit"+File.separator+"problem-cases";
//        String outPutPath = "F:"+File.separator+"data-compiler-testing"+File.separator+"2024-12-16cpp-gcc-llvm-testsuit"+File.separator+"problem-cases"+File.separator+"data";

//        String directoryPath = "F:"+File.separator+"data-compiler-testing"+File.separator+"2024-12-16cpp-gcc-llvm-testsuit"+File.separator+"test-suit-all";
//        String outPutPath = "F:"+File.separator+"data-compiler-testing"+File.separator+"2024-12-16cpp-gcc-llvm-testsuit"+File.separator+"test-suit-all-slice"+File.separator+"0319";
...
//        String outPutPath = "F:"+File.separator+"data-compiler-testing"+File.separator+"2024-12-16cpp-gcc-llvm-testsuit"+File.separator+"programs-deleted-failed-slice"+File.separator+"0319";
...
//        String outPutPath = "F:"+File.separator+"data-compiler-testing"+File.separator+"2024-12-16cpp-gcc-llvm-testsuit"+File.separator+"programs-deleted-failed-slice"+File.separator+"0408";
        ...
        String directoryPath = "test\\test";
        String outPutPath = "test\\0625";
        ...
        statisticsMainPrograms(cpList,outPutPath+""+File.separator+"cppMainProgram.txt");//统计main函数不为空的程序数量 并且过滤所有不包含main函数的源程序

        Serializable serializable=new Serializable();
        serializable.objectToFileForSlice(cpList, outPutPath+""+File.separator+"cppProgramList0625.txt");
    ...
    private static void scopeTyping(ScopeBlockForSlice scopeBlock) {
        ...
        scopeBlock.setScopeOut(cp.getMainOut());//在每个作用域模块都加入全局的内容，包含函数声明，结构体，全局变量等内容
        // 检测是否为单行赋值语句（如let语句）
        if (block.trim().startsWith("let")) {
            List<ScopeBlockForSlice> blocks = scopeBlockMap.get("assignment");
            blocks.add(scopeBlock);
            scopeBlockMap.put("assignment", blocks);
            List<String> assignments = cp.getAssignmentList();
            assignments.add(block.trim()); // 添加单行语句到assignment列表
            cp.setAssignmentList(assignments);
...
//        else if(block.startsWith("let")){
//            List<ScopeBlockForSlice> blocks=scopeBlockMap.get("assignment");
//            blocks.add(scopeBlock);
//            scopeBlockMap.put("assignment",blocks);
//            List<String> assignmentas=cp.getAssignmentList();
//            assignmentas.add(block);
//            cp.setAssignmentList(assignmentas);
//        }
        ...
        }else if(block.startsWith("fn")){//更多赋值语句的匹配方法还需要再细化//其实匹配到的是返回值类型为int的函数
            if(block.startsWith("fn main")){
...
//        String filePath = "F:"+File.separator+"data-compiler-testing"+File.separator+"slice.cpp"; // 替换为你的文件路径
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (line.startsWith("let")&&line.endsWith(";")) {//变量初始化或者赋值行
                    // 创建虚拟的ScopeBlockForSlice（假设构造函数接受单行内容）
                    ScopeBlockForSlice assignmentBlock = new ScopeBlockForSlice(line, rootScope);
                    assignmentBlock.setCppProgram(cp);
                    matchedBlocks.add(line);
                    matchedScopeBlocks.add(assignmentBlock);

//                    scopeTyping(assignmentBlock); // 直接分类为assignment
//                    cp.getAssignmentList().add(line); // 添加到程序的assignment列表
                }
                //忽略注释行
                ...
                //如果要开始切分main函数
                if(line.startsWith("fn main")&&mainOutFlag&&currentBlock==null){
                    //增加main-out位置注释
